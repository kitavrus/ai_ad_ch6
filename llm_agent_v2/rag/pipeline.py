...
class IndexingPipeline:
    ...
    def run(self, docs_dir: str, strategy: str, output_path: str,
            extra_files: list[str] | None = None) -> IndexStats:
        """
        ...
        if extra_files:
            for ef in extra_files:
                p = Path(ef)
                if p.is_file() and p not in md_files:
                    md_files = sorted(list(md_files) + [p])
        if not md_files:
