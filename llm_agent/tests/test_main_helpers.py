...
class TestPrintStrategyStatus:
    def test_sliding_window_no_summary(self, capsys):
        from llm_agent.chatbot.main import _print_strategy_status
        state = _make_state()
...
class TestBuildPlanPrompt:
    def test_contains_description(self):
        from llm_agent.chatbot.main import _build_plan_prompt
...
class TestParseStepsFromLlmResponse:
    def test_layer1_clean_json_array(self):
        from llm_agent.chatbot.main import _parse_steps_from_llm_response
...
class TestValidateSteps:
    def test_empty_list_returns_none(self):
        from llm_agent.chatbot.main import _validate_steps
...
class TestPrintTaskPlan:
    def test_prints_current_step_marker(self, capsys, monkeypatch, tmp_path):
        from llm_agent.chatbot.main import _print_task_plan
        from llm_agent.chatbot.models import TaskPlan, TaskStep
        from datetime import datetime
...
class TestGetActivePlan:
    def test_returns_none_when_no_active_task(self):
        from llm_agent.chatbot.main import _get_active_plan
        state = _make_state()
...
class TestAdvancePlan:
    def test_advances_to_next_step(self, monkeypatch, tmp_path, capsys):
        from llm_agent.chatbot.main import _advance_plan
        monkeypatch.chdir(tmp_path)
        state = _make_state()
...
class TestBuildAgentStateVars:
    def test_empty_state_no_memory(self):
        from llm_agent.chatbot.main import _build_agent_state_vars
        state = _make_state()
        state.memory = None
...
class TestApplySessionDataValueError:
    def test_invalid_strategy_kept(self):
        from llm_agent.chatbot.main import _apply_session_data
        state = _make_state()
        state.context_strategy = ContextStrategy.SLIDING_WINDOW
...
class TestApplyInlineUpdatesExtra:
    ...
    def test_branch_uses_existing_checkpoint(self, capsys):
        from llm_agent.chatbot.main import _apply_inline_updates
        from llm_agent.chatbot.context import create_checkpoint
        state = _make_state()
