...
from llm_agent.chatbot.memory import (
    LongTermMemory,
    Memory,
    MemoryFactor,
    ShortTermMemory,
...
class TestMemoryStorage:
    def test_save_and_load_short_term(self, tmp_path, monkeypatch):
        ...
        from llm_agent.chatbot.memory_storage import load_short_term_last, save_short_term
    ...
    def test_save_and_load_working_memory(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import load_working_memory, save_working_memory
    ...
    def test_save_and_load_long_term(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import load_long_term, save_long_term
    ...
    def test_load_short_term_returns_none_missing(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import load_short_term_last
    ...
    def test_load_working_memory_returns_none_missing(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import load_working_memory
    ...
    def test_load_long_term_returns_none_missing(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import load_long_term
        assert load_long_term("nonexistent", profile_name="test") is None

    def test_export_and_import_state(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import export_memory_state, import_memory_state
        st = {"messages": []}
    ...
    def test_get_memory_stats_empty(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import get_memory_stats
    ...
    def test_get_memory_stats_after_save(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        from llm_agent.chatbot.memory_storage import get_memory_stats, save_working_memory
