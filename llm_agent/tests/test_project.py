...
from llm_agent.chatbot.models import Project, SessionState, TaskPhase, TaskPlan
...
class TestProjectStorage:
    def test_save_and_load(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import save_project, load_project
    ...
    def test_load_nonexistent_returns_none(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import load_project
        monkeypatch.chdir(tmp_path)
        assert load_project("no-such-id", "test") is None

    def test_list_projects(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import save_project, list_projects
        monkeypatch.chdir(tmp_path)
    ...
    def test_list_empty(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import list_projects
        monkeypatch.chdir(tmp_path)
        assert list_projects("test") == []

    def test_delete_project(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import save_project, load_project, delete_project
        monkeypatch.chdir(tmp_path)
    ...
    def test_delete_nonexistent_returns_false(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import delete_project
        monkeypatch.chdir(tmp_path)
        assert delete_project("no-such-id", "test") is False

    def test_load_corrupt_json_returns_none(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.project_storage import load_project, _project_path
        monkeypatch.chdir(tmp_path)
        pid = uuid.uuid4().hex
...
class TestHandleProjectCommand:
    def test_new_creates_project(self, monkeypatch, tmp_path, capsys):
        from llm_agent.chatbot.main import _handle_project_command
        monkeypatch.chdir(tmp_path)
        state = _make_state()
    ...
    def test_list_shows_projects(self, monkeypatch, tmp_path, capsys):
        from llm_agent.chatbot.main import _handle_project_command
        from llm_agent.chatbot.project_storage import save_project
        monkeypatch.chdir(tmp_path)
        state = _make_state()
        p = _make_project()
...
class TestSessionStateParallelTasks:
    ...
    def test_session_persists_active_task_ids(self, monkeypatch, tmp_path):
        from llm_agent.chatbot.main import _build_session_payload, _apply_session_data
        monkeypatch.chdir(tmp_path)
        state = _make_state()
...
class TestPlanFlagParsing:
    def test_parse_plan_flag_present(self):
        from llm_agent.chatbot.main import _parse_plan_flag
    ...
    def test_builder_with_plan_flag(self, monkeypatch, tmp_path, capsys):
        ...
        from llm_agent.chatbot.main import _handle_agent_command
...
class TestProjectPlanInvariantIntegration:
    ...
    def _setup_project_with_plan(self, tmp_path, state):
        ...
        from llm_agent.chatbot.project_storage import save_project
        from llm_agent.chatbot.task_storage import save_task_plan, save_task_step
        from llm_agent.chatbot.models import StepStatus
        ...
        from llm_agent.chatbot.models import TaskStep
        step1 = TaskStep(
    ...
    def test_full_flow_create_project_add_invariants_run_builder(
        ...
        """
        from llm_agent.chatbot.main import _handle_project_command, _handle_agent_command, _handle_invariant_command
        from llm_agent.chatbot.project_storage import load_project
        from llm_agent.chatbot.task_storage import load_task_plan
        from unittest.mock import patch
        monkeypatch.chdir(tmp_path)
    ...
    def test_builder_with_invariants_uses_correct_plan(self, monkeypatch, tmp_path, capsys):
        ...
        """
        from llm_agent.chatbot.main import _handle_agent_command, _handle_invariant_command
        from llm_agent.chatbot.project_storage import save_project
        from llm_agent.chatbot.task_storage import save_task_plan
        from unittest.mock import patch
        monkeypatch.chdir(tmp_path)
